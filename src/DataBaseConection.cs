...
namespace 图书管理系统
{
    class DataBaseConection
    {
        ...
        {
            string LoginRecordCommandText="insert into loginrecord(UserName,LoginTime)" +"values" + "('"+name+ "','" + LoginTime + "')";
        ...
        public DataTable loanRecord(string name)
        {
            ...
            string CommandText = "select book_number,book_title,borrower,borrow_time,due_date,returned,return_time,isbroken, islost, fine from loaninfo" +" where borrower='"+name+"'";
        ...
        {
            DataTable books;
            string showBookCommandText = "select * from booksinfo";
        ...
        {
            string seekBookByNameCommand = "select * from booksinfo where book_title='" + bookName + "'";
            DataTable bookData;
        ...
        {
            string seekBookByNameCommand = "select * from booksinfo where category='" + bookClass + "'";
        ...
        public string[]  bookinfos(string bookName)
        {
            ...
            string bookDigestCommand = "select * from booksinfo where book_title='" + bookName + "'";
            ...
            using (MySqlConnection bookDigestConnection = new MySqlConnection(connectionString))
            {
                ...
                {
                    bookinfos[0]=mySqlDataReader["description"].ToString();
                    bookinfos[1] = mySqlDataReader["book_title"].ToString();
                    bookinfos[2] = mySqlDataReader["category"].ToString();
                    bookinfos[3] = mySqlDataReader["publisher"].ToString();
                    bookinfos[4] = mySqlDataReader["author"].ToString();
                    bookinfos[5] = mySqlDataReader["publication_date"].ToString();
                    bookinfos[6] = mySqlDataReader["book_number"].ToString();
                }
            }
            return bookinfos;
        }
        ...
        {
            string borrowBookCommand = "insert into loaninfo(book_number,book_title,borrower,borrow_time,due_date,returned)" +
        ...
        public bool[] returnBook(string returnTime,string borrowTime, string userName )
        {
            ...
            string returnCommandText = "update loaninfo set returned='是',return_time='" + 
                returnTime + "' where borrow_time='" + borrowTime + "'" + " and borrower='"+userName+"'";
        ...
        public DataTable seekUser(string userClass,string value)
        {
            string seekUserByClassCommandText = "select Power 权限 , UserName 用户名,StudentName 姓名,Sex 性别,WorkUnit 学院 ,PhoneNumber 电话号码," +
                "UserID book_number,SigninTime 注册时间 from users " +
        ...
        {
            string bookInfoChangeCommandText="update booksinfo set book_title='"+ bookName + "',category='"+ bookClass + "',publisher='"+ press + "'," +
                "author='"+ author + "',publication_date='"+ publishTime + "' where book_number='"+ bookNumber + "'";
        ...
        {
            string bookDeleteCommand = "delete from booksinfo where book_title='" + name + "'";
            ...
            using (MySqlConnection rollGRANTConnection = new MySqlConnection(connectionString))
            {
               ...
               //读取是否在管理员下
                ...
                using (MySqlDataReader getUserPasswordDataReader = getUserPasswordCommand.ExecuteReader())
                {
                    ...
                    {
                        grantUserCommand = "insert into admin(AdminName,Password)" +"value('" + name + "','" + getUserPasswordDataReader["Password"].ToString() + "')";
        ...
        {
            bool IsBookAdd;
            string addNewBookCommandText = "insert into booksinfo (book_title,category,language,original_title,book_number,call_number," +
                "author,isbn,translator,publication_date,publisher,description)" +
                "value('"+ bookName + "','"+ bookClass + "','"+ language + "','"+ oldName + "'," +
                "'"+ bookNumber + "','"+ callNumber + "','"+ author + "','"+ iSBN + "','"+ translator + "','"+publish+"'," +
                "'"+ press + "','"+ content+ "')";
            string IsBookEixstedCommandText = "select * from booksinfo where book_title='" +bookName + "'";
        ...
        {
            DataTable books;
            string showBookCommandText = "select book_number,book_title,borrower,borrow_time,due_date,returned,return_time,isbroken,islost,fine " +
                "from loaninfo";
        ...
        {
            string seekBookByNameCommand = "select book_number,book_title,borrower,borrow_time,due_date,returned,return_time,isbroken,islost,fine" +
                " from loaninfo where book_title='" + bookName + "'";
        ...
        {
            string seekBookByNameCommand = "select book_number,book_title,borrower,borrow_time,due_date,returned,return_time,isbroken,islost,fine" +
                " from loaninfo where borrower='" + UserName + "'";
        ...
        public bool borrowAdd(string bookName, string name, string borrowTime, string bookNumber)
        {
            ...
            string borrowBookCommand = "insert into loaninfo(book_number,book_title,borrower,borrow_time,due_date,returned)" +
                 "values('" + bookNumber + "','" + bookName + "','" + name + "','" + borrowTime + "','一个月','否')";
            string seekBookCommand = "select * from loaninfo where book_title='" + bookName + "'"+ " and borrower='"+ name + "'"
                + " and returned='否'";
            ...
            using (MySqlConnection userAddConnection = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlDataReader mySqlDataReader = SeekCommand.ExecuteReader())
                {
                    ...
                    if (mySqlDataReader.Read())
                    {
                        if (mySqlDataReader["returned"].ToString() == "否" )
                        {
                            IsborrowAdd = false;
                        }
                        else
